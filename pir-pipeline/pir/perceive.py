    ...
    def extract_frames_fast_ffmpeg(
        ...
        try:
            ...
            cmd = [
                ...
                '-i', video_path,               # Input
                '-vf', 'fps=10,scale=640:-1',     # 1 frame cada 'step' frames
                '-q:v', '2',                    # Calidad alta
    ...
    # GROUNDING DINO
    # ══════════════════════════════════════════════════════════
    def build_dino_prompt(obj1: str, obj2: str, include_hands: bool = True) -> str:
        """
        Prompt configurable. En etapas finales de ensamblaje donde solo quedan piezas,
        puedes quitar 'hands' para reducir falsos positivos.
        """
        objects = list({obj1, obj2})
        
        # Base: objetos con variantes
        parts = []
        for obj in objects:
            parts.extend([obj, f"{obj} component", f"toy {obj}"])
            # Si es conector, añade términos de unión
            if any(x in obj for x in ["connector", "pin", "joint"]):
                parts.extend(["connector", "fastener", "joint", "coupling"])
        
        # Opcional: manos (útil en Assembly101, no tanto en dark factories)
        if include_hands:
            parts.extend(["human hand", "person", "operator hand", "finger"])
        
        # Filtro de calidad: quita términos muy genéricos que confunden a DINO
        # Ej: "assembly component" es muy vago, mejor ser específico
        return " . ".join(dict.fromkeys(parts)) + " ."  # dict.fromkeys preserva orden sin duplicados
    ...
    def run_dino(
        ...
        for label, score, box in zip(
            results["text_labels"], results["scores"], results["boxes"].cpu()
        ):
    ...
    def run_sam2(
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for i, frame in enumerate(frames):
                ...
                cv2.imwrite(os.path.join(tmp, f"{i:05d}.jpg"), frame_small)
    ...
    def perceive(
        ...
        if max_frames is None:
            max_frames = min(600, int((end - start) / video.fps * 10))
