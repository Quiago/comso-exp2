...
# ── Model weights ─────────────────────────────────────────
echo ""
echo "[3/4] Downloading Cosmos-Reason2-2B (~16GB, cached after first run)..."
cd ..
export HF_TOKEN=$(grep HF_TOKEN .env | cut -d '=' -f2)
hf auth login --token $HF_TOKEN
hf download nvidia/Cosmos-Reason2-2B --cache-dir ~/.cache/huggingface/hub --exclude "*.pt" "original/*"
echo ""
